-- Contraintes SQL complementaires (voir ADR-023 et
-- prisma/manual-constraints/001_check_constraints.sql pour le detail des
-- decisions). Noms de tables/colonnes verifies contre la migration
-- initiale reellement generee (20260718115033_initial).
...
-- Adresse : exactement un proprietaire (utilisateur OU entreprise)
...
-- Panier : etats mutuellement exclusifs (voir ADR et
-- tests/e2e/raw-sql-proof/run.mjs pour la preuve executee) — utilisateur
-- particulier (avec ou sans entreprise associee, exception metier
-- justifiee) OU invite protege par jeton, jamais les deux a la fois, et
-- jamais aucune identite.
